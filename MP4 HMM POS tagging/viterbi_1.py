...
def cal_pr(dict, alpha):
    ...
    for i in dict:
        pr_dic[i] = {'unk' : math.log((0 + alpha) / (sum(dict[i].values())+alpha*(len(dict[i])+1)))}
        for j in dict[i]:
            pr_dic[i][j] = math.log((dict[i][j] + alpha) / (sum(dict[i].values())+alpha*(len(dict[i])+1)))
